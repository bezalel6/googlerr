...
export function refreshDiagnostics(
  ...
  const errs = vscode.languages.getDiagnostics(doc.uri);
  errs.forEach((err) => {
    if (err.code !== EMOJI_MENTION) console.log(err);
    if (!err.severity) {
      err.code = EMOJI_MENTION;
    }
  });

  // errs[0] ? (errs[0].message = "meow") : 1;
  // emojiDiagnostics.set(doc.uri, errs);
